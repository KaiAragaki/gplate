...
my_plate |> gp_sec("condition", nrow = 16, ncol = 3) |> 
  gp_plot(as.factor(condition)) # To make sections a bit more visible
```
...
```{r fig.width=5.6, fig.height=3.25}
my_plate |> gp_sec("condition", ncol = 3) |> gp_plot(as.factor(condition))
```
...
```{r fig.width=5.6, fig.height=3.25}
my_plate |> gp_sec("condition", 8, 11) |> gp_plot(as.factor(condition))
```

```{r fig.width=5.6, fig.height=3.25}
my_plate |> gp_sec("condition", 8, 11, start_corner = "br") |> gp_plot(as.factor(condition))
``` 
...
```{r fig.width=5.6, fig.height=3.25}
my_plate |> gp_sec("condition", 8, 11, flow = "col") |> gp_plot(as.factor(condition))
```
...
```{r fig.width=5.6, fig.height=3.25}
borderless <- my_plate |> gp_sec("no_border", margin = 1, 14, 22)

borderless |> gp_plot(no_border)
```
...
```{r fig.width=5.6, fig.height=3.25}
borderless |> gp_sec("primers", ncol = 3) |> gp_plot(as.factor(primers))
```
...
```{r fig.width=5.6, fig.height=3.25}
my_plate |> 
  gp_sec("primers", nrow = 3, ncol = 27, wrap = TRUE) |> 
  gp_plot(as.factor(primers))
```
...
gp(20, 20) |> 
  ...
  gp_plot(as.factor(inner)) + ggplot2::theme(legend.position = "none")
```
