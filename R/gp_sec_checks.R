...
check_if_flow_and_custom_dims <- function(flow, nrow, ncol) {
  if (flow == "row" & length(nrow != 1)) {
    rlang::abort(message = c("length(`nrow`) is not equal to 1, but `flow` == \"row\"",
                             i = "nrow must be contant if you wish to wrap along rows",
                             i = "(ncol can vary, though)"))
  }

  if (flow == "col" & length(ncol != 1)) {
    rlang::abort(message = c("length(`ncol`) is not equal to 1, but `flow` == \"col\"",
                             i = "ncol must be contant if you wish to wrap along columns",
