...
unroll_sec_dim_along_parent <- function(gp, dim, flow, wrap) {
  ...
  flow_sec_rel_par <- rlang::sym(paste0(".", flow, "_sec_rel_par"))
  ...
  non_flow_sec_rel_par <- rlang::sym(paste0(".", non_flow, "_sec_rel_par"))
  ...
  if (wrap) {
    ...
    gp$well_data <- gp$well_data |>
      dplyr::group_by(.data$.sec, {{ flow_sec_rel }}) |>
    ...
    if (!is_fwd(gp, non_dim)) {
      gp$well_data <- gp$well_data |>
        dplyr::mutate({{ non_flow_sec }} := {{ non_flow_sec }} * -1 + 1 + ndim_sec)
    }
