make_sec <- function(gp, flow) {
  ...
  fake_sec <- expand_grid(.row_sec_par = gp$row_coord_map$.row_sec_par, .col_sec_par = gp$col_coord_map$.col_sec_par)

  fake_sec_full <- fake_sec |>  left_join(gp$row_coord_map) |> left_join(gp$col_coord_map)

  fake_sec_full <- fake_sec_full |>
    dplyr::arrange(.row_sec_rel, .col_sec_rel) |>
  ...
  gp$well_data <- gp$well_data |> left_join(fake_sec_full)
